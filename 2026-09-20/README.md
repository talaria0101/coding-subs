...
| Field | Value |
|---|---|
| **BEST DEAL FOUND (overall, unchanged)** | **Z.ai GLM Coding Plan — Lite tier, $18/month** |
| **RE-VERIFIED** | 2026-09-20 first-party docs: same $18/$72/$160 monthly, same official allowance tables (Lite GLM-5.3 @95% cache: **48–97M tokens/week ≈ 208–420M/month**) |
| **WHAT ENDED TODAY** | The GLM-5.3-Flash campaign (Sep 3–20): unlimited Flash in ZCode 23:00–09:00 SGT and doubled quota elsewhere lapse after Sep 20 — the 09-13 report's "632M–1,264M Flash-month" bonus figures expire with it |
| **CHEAPEST NEW ENTRY** | **Command Code Go — $1/month** ($10 credits, up to ~$20 usage with deals, ~15K requests, up to 1M context) — a real subscription vendor found by category enumeration, not a relay |
| **CHEAPEST FRONTIER QUALITY** | ChatGPT Plus $20 (GPT-5.6 Sol II 46.97) and Claude Pro $20 (1M-context Claude Code) — both re-verified, both unchanged |
| **BEST FREE** | Google Antigravity free tier — Gemini 3.8/3.7/3.6 Flash + 3.1 Pro + Claude Sonnet 4.6 & Opus 4.6 (thinking) + gpt-oss-120b, weekly-refreshed quota; re-verified under the "Antigravity 2.0" rebrand |
| **CONFIDENCE** | HIGH on all prices (first-party pages snapshotted today); MEDIUM on quota fairness for the new ladders (Kiro/Devin/Droid credits have no token tables); the Command Code "effective usage" deal multiples are ADVERTISED, not measured |
...
Muse Code (3 tiers, prices still onboarding-only), Alibaba Model Studio Coding Plan Pro $50
...
|---|---|---|---|---|---|
| 1 | **Z.ai GLM Coding Plan Lite** | **$18** | GLM-5.3 (44.78) | Official 208–420M tokens/mo (GLM-5.3 @95% cache), 1M ctx, works in Claude Code/OpenCode/Cline/etc; every number first-party | Flash campaign ended today; GLM-5.3 below Opus-5 class; vision via MCP only |
| 2 | **Command Code GOAT** | **$10** | per-model allowances incl. GPT-5.6 Sol (46.97) | $10 buys $70 of earmarked credits (+ deals to ~$100 usage); ~75K requests; 1M ctx; $1 Go tier is the cheapest paid entry anywhere | Deal multiples ADVERTISED; "+ processing fee"; young vendor, no third-party track record |
| 3 | **ChatGPT Plus (Codex)** | $20 | GPT-5.6 Sol (46.97) | Frontier quality, official message tables (Sol 10–100/5h), flex credits with an explicit rate card | Message-based; weekly caps; GPT-5.5 retires Oct 14 (migration churn) |
| 4 | **Claude Pro** | $20 | Opus 5 (50.78) / Fable 5.1 (53.35) | 1M context in Claude Code (documented); the only true-frontier escape hatch at $20 | Weekly limits now ~17% below the spring promo level; Pro needs usage credits enabled for Opus 1M |
| 5 | **Google AI Pro (+ Antigravity 2.0)** | $19.99 | Gemini 3.8 Flash (40.93) + Claude Sonnet/Opus 4.6 (thinking) | Three labs in one sub; best multimodal agent; free tier exists | Quotas are opaque compute units; 3.8 Flash API intro price doubles Jan 1, 2027 (subscription pricing so far unaffected) |
| 6 | **Claude Max 5x** | $100 | Opus 5 / Fable 5.1 | The real Opus-5-class experience, 1M verified, priority access | ~5x Pro ESTIMATED ~50M+/mo; 5.5x the price of #1 |
| 7 | **Muse Code (Meta) High/Power** | ~$20–100 | Muse Spark 1.3 (48.09) | Highest-II subsidized access; 1M ctx; image+video | Prices still shown only at onboarding; young product; telemetry caveats from the 09-13 pass stand |
| 8 | **Z.ai GLM Coding Plan Pro** | $72 | GLM-5.3 (44.78) | Official 1.26–2.51B tokens/mo estimate — heavy-usage king per dollar | Same ceiling as #1 |
...
| 10 | **Kiro Pro / Pro Max** | $20 / $100 | premium models (unspecified credits) | Clear credit ladder with $0.04 add-on pricing — the most honest metered ladder on the West coast | Credit-to-token value unpublished; capacity UNKNOWN |

Dropped from the old top-10: Trae Pro (doubled in price), Copilot Pro+ (breadth at API-parity
credits; the new Command Code GOAT takes the multi-model slot at half the price), MiniMax Plus
(capacity still unpublished; M3 slipped to II 29.22).
...
7. **Cerebras restock watch** — still sold out at $50/$200 on GLM-4.7; if it restocks on GLM-5.3 at
...
1. **Cheapest viable:** Antigravity free ($0); cheapest guaranteed-capacity viable: **GLM Lite $18**.
2. **Best value overall:** **GLM Coding Plan Lite** — 20–40× Claude Pro capacity at comparable
   money, first-party quota tables, re-verified today.
3. **Cheapest paid entry:** **Command Code Go, $1** — with the explicit caveat that its deal
   multiples are advertisements until independently measured.
4. **Best under $15:** Command Code GOAT $10 (if the deals hold) or Zed Pro $10 + any BYOK sub.
5. **Best at $20:** Claude Pro (quality ceiling + documented 1M) and ChatGPT Plus (frontier Sol);
   pick by harness preference.
6. **Best heavy-usage:** GLM Pro $72 (1.26–2.51B tokens/mo official estimate); runner-up Codex Pro 20x $200.
7. **Best multi-model:** Command Code GOAT (per-model allowances across 5+ labs, $10) replaces
   Copilot Pro+ ($39 of API-parity credits); free pick remains Antigravity.
...
12. **Maximum tokens/$:** GLM Lite quota on GLM-5.3-Flash ≈ $0.014/M tokens at the low end (the
    campaign's extra multiplier is gone as of today; these are the standard-rate figures).
...
1. **Unchanged core: GLM Coding Plan Lite ($18) + Claude Pro ($20).** GLM carries the bulk with
   ~4× headroom over a 52.5M/mo workload; Claude Pro remains the verified-1M true-frontier escape
   hatch. Total $38/mo.
2. **New experiment slot: add Command Code Go for $1** (or GOAT $10 if you will actually spend the
   $70 of allowances). Treat month one as a measurement, not a commitment: log the effective usage
   against the advertised "up to" numbers and the "+ processing fee".
3. **If you were riding the Claude promo levels:** re-run `/usage` against your August numbers
...
4. **Do not buy:** anything from the relay/reseller sponsor tables, however many decimal places
...
- **Known gaps:** Kimi's new-tier USD prices sit behind a client-rendered paywall (structure
  VERIFIED, price THIRD-PARTY); Cursor Pro+/Ultra behind a JS tab; Qoder and CodeBuddy still
  JS-rendered shells; Command Code's deal multiples and Kimi/Kiro/Devin/Droid credit-to-token
  values are unpublished; Google's quotas remain opaque compute units. The AA leaderboard exposes
  only II for the top 20 — deeper model fields are week-old by design and labeled.
